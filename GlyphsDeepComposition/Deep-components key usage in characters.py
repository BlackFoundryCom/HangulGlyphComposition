...
minDiam = min([k for k in compo2char.values()])
...
s = W-margins*2
for i in range(s):
    colred = ((minDiam+i)/(maxDiam+i))/divide
    colgreen = ((minDiam+i)/(maxDiam+i))/divide
    colblue = ((minDiam+i)/(maxDiam+i))/divide
...
fill(1)
text(str(minDiam), (margins, margins*3.5), align='left')
text(str(int(minDiam+(maxDiam-minDiam)/2)), (W/2, margins*3.5), align='center')
